...
_add_warning_to_bashrc() {
    if [[ "$(grep -q print-missing-gitlab-token-warning ~/.bashrc; echo $?)" == 1 ]]; then
        echo '/bin/bash /opt/devcontainer/bin/print-missing-gitlab-token-warning.sh' >> ~/.bashrc;
    fi
}

_remove_warning_from_bashrc() {
    if [[ "$(grep -q print-missing-gitlab-token-warning ~/.bashrc; echo $?)" == 0 ]]; then
        sed -i 's@/bin/bash /opt/devcontainer/bin/print-missing-gitlab-token-warning.sh@@g' ~/.bashrc;
    fi
}

if [[ "${CODESPACES:-false}" == true ]]; then
    if [[ -z "$GITLAB_TOKEN" ]]; then
        /opt/devcontainer/bin/print-missing-gitlab-token-warning.sh;
        _add_warning_to_bashrc;
        exit 0;
    fi
